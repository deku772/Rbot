...
abstract class HomeActivity : AppBarActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.action_stop -> {
                RbotAnalytics.logEvent(this, "automation_shizuku_stop_menu_tap")
                if (!Shizuku.pingBinder()) {
                    RbotAnalytics.logEvent(this, "automation_shizuku_stop_blocked", "reason", "binder_not_ready")
                    return true
                }
                MaterialAlertDialogBuilder(this)
                    .setMessage(R.string.dialog_stop_message)
                    .setPositiveButton(android.R.string.ok) { _: DialogInterface?, _: Int ->
                        RbotAnalytics.logEvent(this, "automation_shizuku_stop_confirm_tap")
                        try {
                            Shizuku.exit()
                            RbotAnalytics.logEvent(this, "automation_shizuku_stop_completed")
                        } catch (e: Throwable) {
                            RbotAnalytics.logEvent(this, "automation_shizuku_stop_failed")
                        }
                    }
                    .setNegativeButton(android.R.string.cancel) { _: DialogInterface?, _: Int ->
                        RbotAnalytics.logEvent(this, "automation_shizuku_stop_cancel_tap")
                    }
                    .show()
                RbotAnalytics.logEvent(this, "automation_shizuku_stop_dialog_shown")
                true
            }
