...
class RequestPermissionActivity : AppActivity() {
    ...
    private fun checkSelfPermission(): Boolean {
        ...
        RbotAnalytics.logEvent(this, "automation_shizuku_permission_limited_dialog_shown")
        return false
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!waitForBinder()) {
            RbotAnalytics.logEvent(this, "automation_shizuku_permission_flow_failed", "reason", "binder_timeout")
            finish()
            return
        }
        ...
        if (uid == -1 || pid == -1 || ai == null) {
            RbotAnalytics.logEvent(this, "automation_shizuku_permission_flow_failed", "reason", "invalid_request")
            finish()
            return
        }
        if (!checkSelfPermission()) {
            RbotAnalytics.logEvent(this, "automation_shizuku_permission_denied", "mode", "limited")
        ...
        RbotAnalytics.logScreen(this, "automation_shizuku_permission_request", "RequestPermissionActivity")
        val binding = ConfirmationDialogBinding.inflate(layoutInflater).apply {
            button1.setOnClickListener {
                RbotAnalytics.logEvent(this@RequestPermissionActivity, "automation_shizuku_permission_granted", "mode", "always")
                setResult(uid, pid, requestCode, allowed = true, onetime = false)
                dialog.dismiss()
            }
            button3.setOnClickListener {
                RbotAnalytics.logEvent(this@RequestPermissionActivity, "automation_shizuku_permission_denied", "mode", "onetime")
                setResult(uid, pid, requestCode, allowed = false, onetime = true)
                dialog.dismiss()
            }
        ...
        dialog.show()
        RbotAnalytics.logEvent(this, "automation_shizuku_permission_dialog_shown")
    }
}
