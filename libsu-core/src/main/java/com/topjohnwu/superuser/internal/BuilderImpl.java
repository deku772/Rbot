...
public final class BuilderImpl extends Shell.Builder {
    ...
    private ShellImpl start() {
        ...
        // KernelSU compatibility: su may be hidden from app's mount namespace,
        // but can be accessed via nsenter into init's mount namespace.
        // This requires the app to have root granted in KernelSU manager.
        if (shell == null && !hasFlags(FLAG_NON_ROOT_SHELL)) {
            try {
                shell = exec("nsenter", "--mount=/proc/1/ns/mnt", "--", "su");
                if (!shell.isRoot()) {
                    shell = null;
                }
            } catch (NoShellException ignore) {}
